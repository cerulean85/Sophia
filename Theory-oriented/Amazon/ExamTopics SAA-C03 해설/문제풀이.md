...
# 515번
### 문제 515
회사는 온프레미스 애플리케이션을 AWS로 마이그레이션하고 있습니다. 회사는 Amazon Redshift를 솔루션으로 사용하고자 합니다.

어떤 사용 사례가 이 시나리오에서 Amazon Redshift에 적합합니까? (세 가지를 선택하십시오.)

A. 전통적인, 컨테이너화된, 이벤트 기반 애플리케이션으로 데이터를 액세스하기 위한 데이터 API 지원
B. 클라이언트 측 및 서버 측 암호화 지원
C. 애플리케이션이 활성화되지 않은 동안 지정된 시간 동안 분석 워크로드 구축
D. 백엔드 데이터베이스에 대한 압력을 줄이기 위해 데이터 캐싱
E. 페타바이트 규모의 데이터와 분당 수천만 건의 요청을 지원하기 위해 글로벌 확장
...
### 해설
- **B. Supporting client-side and server-side encryption**
  - **적합성**: Amazon Redshift는 클라이언트 측 및 서버 측 암호화를 지원하여 데이터 보안을 강화합니다.

- **C. Building analytics workloads during specified hours and when the application is not active**
  - **적합성**: Amazon Redshift는 대규모 데이터 분석 워크로드를 처리하는 데 적합하며, 애플리케이션이 활성화되지 않은 동안 지정된 시간 동안 분석 작업을 수행할 수 있습니다.

- **E. Scaling globally to support petabytes of data and tens of millions of requests per minute**
  - **적합성**: Amazon Redshift는 페타바이트 규모의 데이터를 처리하고, 분당 수천만 건의 요청을 지원할 수 있는 글로벌 확장성을 제공합니다.

### 다른 옵션들에 대한 설명
- **A. Supporting data APIs to access data with traditional, containerized, and event-driven applications**
  - Amazon Redshift는 주로 데이터 웨어하우스 솔루션으로 사용되며, 데이터 API를 통한 전통적인, 컨테이너화된, 이벤트 기반 애플리케이션의 데이터 액세스는 다른 서비스가 더 적합할 수 있습니다.

- **D. Caching data to reduce the pressure on the backend database**
  - 데이터 캐싱은 주로 Amazon ElastiCache와 같은 서비스가 더 적합합니다.

- **F. Creating a secondary replica of the cluster by using the AWS Management Console**
  - Amazon Redshift는 기본적으로 고가용성을 제공하지만, 보조 복제본을 생성하는 것은 일반적인 사용 사례가 아닙니다.

따라서, B, C, E 옵션이 Amazon Redshift에 적합한 사용 사례입니다.
