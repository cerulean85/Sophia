...
## 작동 방식
1. **데이터 수집**
   - AWS 리소스, 애플리케이션, 온프레미스 서버에서 메트릭과 로그 데이터 수집
   - CloudWatch Agent를 사용하여 추가 메트릭과 로그 수집 가능
2. **데이터 저장 및 시각화**
   - 수집된 데이터를 CloudWatch에 저장하고, 대시보드를 통해 시각화
3. **모니터링 및 경고 설정**
   - 실시간으로 데이터를 모니터링하고, 특정 조건이 충족될 때 경고 설정
4. **로그 분석**
   - CloudWatch Logs를 통해 수집된 로그 데이터를 저장, 검색, 분석
5. **자동화**
   - CloudWatch 이벤트를 기반으로 AWS Lambda 함수를 트리거하여 자동화된 작업 수행

## 다른 서비스와의 연관성
- **AWS Lambda**: CloudWatch 이벤트를 기반으로 Lambda 함수를 트리거하여 자동화된 작업 수행
- **AWS Systems Manager**: CloudWatch Agent를 자동으로 설치하고 구성하는 데 사용
- **Amazon SNS**: 경고 알림을 전송하는 데 사용
- **AWS CloudTrail**: CloudWatch와 통합하여 AWS 계정 활동을 모니터링

## 구성 요소
- **CloudWatch Metrics**: AWS 리소스 및 애플리케이션의 성능 데이터를 수집하고 모니터링
- **CloudWatch Logs**: 애플리케이션 및 시스템 로그를 수집, 저장, 분석
- **CloudWatch Alarms**: 특정 조건이 충족될 때 알림을 보내는 경고 설정
- **CloudWatch Dashboards**: 실시간 데이터 시각화를 위한 대시보드 생성
- **CloudWatch Events**: 이벤트 기반 작업 자동화를 위한 트리거 설정
- **CloudWatch Agent**: 추가 메트릭과 로그를 수집하여 CloudWatch로 전송

## 사용 사례
- **시스템 성능 모니터링**: CPU, 메모리, 디스크, 네트워크 등의 시스템 메트릭 모니터링
- **애플리케이션 로그 분석**: 애플리케이션 로그 수집 및 CloudWatch Logs에서 분석
- **경고 설정 및 알림**: 특정 조건이 충족될 때 경고를 설정하고 알림 수신
- **자동화된 작업 수행**: CloudWatch 이벤트를 기반으로 자동화된 작업 수행
...
## 기타
  - 계정 없어도 email 주소 입력해서 링크 공유하여 대시보드 접근 가능
