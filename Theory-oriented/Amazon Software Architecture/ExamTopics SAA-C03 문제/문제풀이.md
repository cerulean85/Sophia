...
# 737
다른 보기들이 정답이 될 수 없는 이유는 다음과 같습니다.

### B. **us-east-2에서 eu-west-2로 단방향 복제, eu-west-2에서 ap-southeast-1로 단방향 복제 구성**
- 이 설정은 **단계적 복제**가 발생하므로 모든 데이터가 ap-southeast-1에 복제되기까지 지연이 생길 수 있습니다. 즉, us-east-2에 업로드된 데이터가 ap-southeast-1에 복제되기 전에 eu-west-2를 거쳐야 하므로 복제 지연이 늘어납니다.
- 따라서, **지연 시간을 최소화하고자 하는 요구 사항에 부적합**합니다.

### C. **세 개의 버킷 간 양방향(양방향) 복제 구성**
- 양방향 복제를 설정하면 각 버킷 간에 데이터가 상호 복제되어 불필요한 네트워크 트래픽과 비용이 발생할 수 있습니다.
- 또한, 양방향 복제는 복잡성과 비용이 크며, 문제에서 언급된 요구 사항에서는 **단방향 복제만으로 충분**하기 때문에 과도한 설정입니다.

### E. **S3 Multi-Region Access Point 생성 후 스트리밍 및 업로드 모두에 ARN 사용**
- 이 설정은 모든 요청에 Multi-Region Access Point를 사용하므로 애플리케이션을 **업로드와 스트리밍 양쪽에서 변경해야 합니다**. 문제의 요구사항은 **변경을 최소화하는 것**이기 때문에, 스트리밍에만 Multi-Region Access Point ARN을 사용하도록 설정하는 D 옵션이 더 적합합니다.
- 또한, Multi-Region Access Point는 데이터 읽기 작업(스트리밍)에서는 유용하지만, 모든 업로드 요청을 Multi-Region Access Point로 변경할 경우 개발자들의 업로드 지연 시간이 오히려 늘어날 수 있습니다.

---

이로 인해, A와 D 조합이 최소한의 애플리케이션 변경으로 요구 사항을 충족하는 최적의 선택입니다.
